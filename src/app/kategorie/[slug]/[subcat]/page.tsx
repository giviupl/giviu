...
type Props = { params: Promise<{ slug: string; subcat: string }> };

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug, subcat } = await params;
  const category = await getCategoryBySlug(slug);
  const subcategory = category?.subcategories?.find(s => s.slug === subcat);
  if (!category || !subcategory) return { title: 'Podkategoria nie znaleziona' };
  ...
  return { title, description, alternates: { canonical: `/kategorie/${slug}/${subcat}` }, openGraph: { title, description, url: `/kategorie/${slug}/${subcat}`, images: [{ url: 'https://giviu.pl/og-default.jpg', width: 1200, height: 630, alt: `${subcategory.name} — prezenty firmowe` }] } };
}
...
export default async function SubcategoryPage({ params }: Props) {
  ...
  return (
    <main className={styles['subcategory-page']}>
      ...
      <SubcategoryClient category={category} subcategory={subcategory} products={products} categorySlug={slug} />
    </main>
  );
}
