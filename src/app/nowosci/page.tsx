...
// ============================================
// ZMIANY:
// - Dane z Supabase (client-side fetch, bo cała strona jest 'use client')
// - brand_name zamiast brand
// - Semantyczne: <aside> na filtrach, <ul role="list"> + <li>
// ============================================
...
export default function NowosciPage() {
  ...
  // Fetch products client-side (ta strona jest 'use client' — layout ma metadata)
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    return allProducts.filter(product => {
      if (selectedBrands.length > 0) {
        if (!product.brand_name || !selectedBrands.includes(product.brand_name)) {
          return false;
        }
      }
  ...
  return (
    <main className="nowosci-page">
      <div className="nowosci-container">
        ...
        {/* Header */}
        <header className="nowosci-header">
          <div className="nowosci-title-wrapper">
            <SectionLine spacing="sm" />
            <h1 className="nowosci-title">Nowości</h1>
          </div>
          <p className="nowosci-description">
            Odkryj najnowsze produkty w naszej ofercie. Świeże propozycje od najlepszych marek premium.
          </p>
        </header>

        {/* Loading state */}
        {loading && (
          <div className="nowosci-empty">
        ...
        {!loading && filteredProducts.length > 0 ? (
          <ul role="list" className="nowosci-grid">
          ...
          <div className="nowosci-empty">
            {allProducts.length > 0 ? (
              <>
                ...
                <button onClick={handleClearAll} className="nowosci-link">
                  Wyczyść filtry →
                </button>
              </>
            ) : (
              <>
                <p>Brak nowych produktów. Wróć wkrótce!</p>
                <Link href="/kolekcje" className="nowosci-link">
