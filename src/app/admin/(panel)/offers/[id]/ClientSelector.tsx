...
interface Props {
  ...
  contacts: Contact[];
  onClientChanged: (offerUpdate: Record<string, string | null>, contacts: Contact[]) => void;
}
...
export default function ClientSelector({
  ...
  const handleSelectClient = async (clientId: string) => {
    ...
    if (contactsData.length === 0) {
      // Brak kontaktów — utwórz placeholder z danych klienta
      alert(
    ...
    // Wybierz primary (lub pierwszy)
    const primary = contactsData.find((c) => c.is_primary) || contactsData[0];

    startTransition(async () => {
      const result = await setOfferClientAction(offerId, clientId, {
        name: primary.name,
        email: primary.email,
        phone: primary.phone,
      });
      if (result.ok) {
        // Fetch firmy żeby zaktualizować UI
  ...
  const handleChangeContact = async (contactId: string) => {
    ...
    startTransition(async () => {
      const result = await setOfferClientAction(offerId, currentClientId, {
        name: contact.name,
        email: contact.email,
        phone: contact.phone,
      });
